	...
	try:
		while True:
			try:
				...
				break
			# !-- Yet not fixed, i gave you the solution...
			# !-- what is the difference between datastr and data?
			datastr = data
			# !-- redundant print...
			print data
			# !-- why not using datastr.startswith('xxx')
			# !-- ********** you would never get into the elif
			# Run the code:
			# def check(my_string):
			# 	if my_string[0] == 'x' and my_string[1] == 'x' and my_string[2] == 'x':
			# 		print 'I am here'
			# 	elif my_string == ('xxxq'):
			# 		print 'I am there'
			# check('xxx')
			...
			if datastr[0] == 'x' and datastr[1] == 'x' and datastr[2] == 'x':
				# !-- that would have been an DEBUG print
				print 'Peer: ' + datastr[3:]
				txtDecrypted, sender = decrypt_Obj_Str_2_Txt(datastr[3:])
				# !-- use something like print '%s(%s): %s' % (date.now, sender, message)
